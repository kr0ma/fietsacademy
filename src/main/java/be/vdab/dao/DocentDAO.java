...
public class DocentDAO extends AbstractDAO {
	...
	// return
	// getEntityManager().createQuery("select d from Docent d where d.wedde between ? and ? "
	...
	// resultlist with interval amount of rows to get each time
	public List<Docent> findByWeddeBetween(BigDecimal van, BigDecimal tot,
		...
		return getEntityManager()
				...
				.setHint("javax.persistence.loadgraph", getEntityManager().createEntityGraph("Docent.metCampus"))
				.getResultList();
	}

	/*
	 * find voornamen in a list public List<String> findVoornamen() { return
	 * getEntityManager().createQuery( "select d.voornaam from Docent d",
	 * String.class) .getResultList(); }
	 */

	// find voornamen en ID in a list (useing a valueobject)
